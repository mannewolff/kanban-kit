 ...
 *
 * Board und Spalte stecken bewusst in **einem** Feld statt in zwei parallel geführten: Die Spalte
 * gehört zu ihrem Board, so kann kein widersprüchlicher Zustand („Spalte ohne Board") entstehen.
 */
export interface CardLocation {
  projectId: number
  /** `null`, solange der Projektname noch nachgeladen wird (`useProjectName`). */
  projectName: string | null
  /** Board der Karte — jede Karte liegt auf einem Board (Issue #1202). */
  board: {
    id: number
    name: string
    /**
...
/**
 * Ortspfad einer Karte als Breadcrumb-Segmente: `Projekt / Board / Spalte`. Verlinkt ist, wohin man
 * tatsächlich navigieren kann — die Spalte hat keine eigene Route, ein archiviertes Board keine
 * erreichbare Seite.
 *
 ...
 * „archiviert" nicht an zwei Stellen gepflegt werden müssen.
 */
