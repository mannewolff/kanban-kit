...
/**
 * Verwaltet die Tabelle {@code card_dependency} (zusammengesetzter Schlüssel ohne
 * eigene ID) direkt per SQL.
 */
@Component
class JdbcCardDependencyRepository implements CardDependencyRepository {
    ...
    public void replaceDependencies(long cardId, List<Integer> dependsOnNumbers) {
        ...
        for (Integer number : dependsOnNumbers) {
            jdbc.update("INSERT INTO card_dependency (card_id, depends_on_card_number) VALUES (?, ?)",
                    cardId, number);
        }
    }
