...
class BoardColumnRepositoryAdapter implements BoardColumnRepository {
    ...
    public List<BoardColumn> findByBoardId(long boardId) {
        return jpa.findByBoardIdOrderByPosition(boardId).stream().map(BoardColumnRepositoryAdapter::toDomain).toList();
    }
    ...
    /**
     * Zwei-Phasen-Reindex: erst alle Positionen weit nach oben verschieben (kollisionsfrei
     * gegen den Unique-Constraint), dann die Zielpositionen 0..n-1 setzen.
     */
    @Override
    public void reorder(long boardId, List<Long> orderedColumnIds) {
        ...
        for (int i = 0; i < orderedColumnIds.size(); i++) {
            jdbc.update("UPDATE board_column SET position = ? WHERE id = ? AND board_id = ?",
                    i, orderedColumnIds.get(i), boardId);
        }
    }
    ...
    private static BoardColumn toDomain(BoardColumnEntity e) {
        return new BoardColumn(e.getId(), e.getBoardId(), e.getName(), e.getPosition(), e.getWipLimit());
    }
}
