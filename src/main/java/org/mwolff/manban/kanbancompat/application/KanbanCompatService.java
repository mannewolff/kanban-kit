 ...
 * 5-Spalten-Protokoll (BACKLOG/READY/IN_PROGRESS/IN_REVIEW/DONE) auf ein manban-Board ab
 * und operiert ausschließlich auf dem an das Token gebundenen Board (#44). Rechte laufen
 * über die bestehenden Services (CardService/CommentService → PermissionChecker).
 *
 * <p>Spalten-Mapping: primär per Namensabgleich (Backlog/Ready/In Progress/In Review/Done),
 * sonst positionsbasiert (i-te Spalte → i-ter Kanban-Key). Das Dogfood-Board nutzt die
...
public class KanbanCompatService {
    ...
    public static final List<String> COLUMNS = List.of("BACKLOG", "READY", "IN_PROGRESS", "IN_REVIEW", "DONE");
    ...
    public KanbanCompatService(CardRepository cards, BoardColumnRepository columns, BoardRepository boards,
                               CardService cardService, CommentService commentService,
    ...
    public Map<String, List<Item>> items(KanbanPrincipal principal) {
        ...
        cards.findByBoardId(boardId).stream()
                ...
                .forEach(c -> {
                    ...
                    grouped.get(key).add(new Item(c.id(), c.number(), c.title(), c.description(),
                            key, c.positionInColumn(), c.type() == CardType.EPIC ? "epic" : "card"));
                });
        return grouped;
    }
    ...
    private Map<Long, String> keyByColumn(long boardId) {
        List<BoardColumn> ordered = columns.findByBoardId(boardId).stream()
    ...
    private long columnIdForKey(long boardId, String key) {
        ...
        return keyByColumn.entrySet().stream()
                ...
                .findFirst()
                .orElseThrow(() -> new InvalidKanbanColumnException(
    ...
    public record Item(Long id, int number, String title, String body, String column, int position, String type) {
    }
