 ...
 *
 * <p>Optional an ein Projekt + Board gebunden ({@code projectId}/{@code boardId}):
 * ein solches Token adressiert genau dieses Board (Kanban-Compat-API, #45), ähnlich
 * einem GitHub-Fine-grained-PAT. Sind beide {@code null}, ist das Token ungebunden.
 *
...
public record AccessToken(
    ...
    public AccessToken withLastUsedAt(Instant when) {
        return new AccessToken(id, userId, projectId, boardId, name, tokenHash, displayName, createdAt, when, revoked);
    }

    public AccessToken asRevoked() {
        return new AccessToken(id, userId, projectId, boardId, name, tokenHash, displayName, createdAt, lastUsedAt, true);
    }
}
