...
/**
 * Security-Slice: volle Filterkette gegen echtes Postgres. Deckt Login (Cookie),
 * /api/me, falsche Daten (401), unverifiziert (403), Default-Deny ohne Cookie (401)
 * und Logout (Cookie-Löschung) ab.
 */
...
class AuthSessionIT {
    ...
    void seedUsers() {
        if (users.findByEmail("verified@example.com").isEmpty()) {
            users.save(new AppUser(null, "verified@example.com", passwordEncoder.encode(PASSWORD),
                    "Verified", true, PlatformRole.USER));
        }
        if (users.findByEmail("unverified@example.com").isEmpty()) {
            users.save(new AppUser(null, "unverified@example.com", passwordEncoder.encode(PASSWORD),
                    "Unverified", false, PlatformRole.USER));
        }
    }
