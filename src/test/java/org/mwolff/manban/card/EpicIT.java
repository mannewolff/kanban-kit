...
class EpicIT {
    ...
    private long createCard(Cookie session, long boardId, long columnId, String title) throws Exception {
    ...
    private long createEpic(Cookie session, long boardId, String title, String shortcode) throws Exception {
        String body = mvc.perform(post("/api/boards/" + boardId + "/cards").cookie(session)
                        .contentType("application/json")
                        .content("{\"type\":\"EPIC\",\"title\":\"%s\",\"shortcode\":\"%s\"}".formatted(title, shortcode)))
    ...
    private void assignParent(Cookie session, long cardId, Long parentId) throws Exception {
        ...
        mvc.perform(patch("/api/cards/" + cardId + "/parent").cookie(session)
                        .contentType("application/json").content("{\"parentId\":%s}".formatted(parent)))
                .andExpect(status().isOk());
    }

    private JsonNode epics(Cookie session, long boardId) throws Exception {
        return json.readTree(mvc.perform(get("/api/boards/" + boardId + "/epics").cookie(session))
                .andExpect(status().isOk()).andReturn().getResponse().getContentAsString());
    }

    private JsonNode boardCards(Cookie session, long boardId) throws Exception {
        return json.readTree(mvc.perform(get("/api/boards/" + boardId + "/cards").cookie(session))
                .andExpect(status().isOk()).andReturn().getResponse().getContentAsString());
    }
    ...
    void epicProgressReflectsChildrenInDone() throws Exception {
        ...
        mvc.perform(post("/api/cards/" + c1 + "/move").cookie(alice)
                        .contentType("application/json")
    ...
    void epicsAreNotMovableOnTheBoard() throws Exception {
        ...
        long epic = createEpic(alice, boardId, "E", "E");
        mvc.perform(post("/api/cards/" + epic + "/move").cookie(alice)
                        .contentType("application/json")
                        .content("{\"columnId\":%d,\"position\":0}".formatted(done)))
