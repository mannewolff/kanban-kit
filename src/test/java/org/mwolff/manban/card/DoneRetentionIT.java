...
class DoneRetentionIT {
    ...
    @Autowired
    private DoneRetentionService retention;
    @Autowired
    ...
    private long createCard(Cookie session, long boardId, long columnId, String title) throws Exception {
        return json.readTree(mvc.perform(post("/api/boards/" + boardId + "/cards").cookie(session)
    ...
    private void setMovedToDone(long cardId, Instant when) {
        jdbc.update("UPDATE card SET moved_to_done_at = ? WHERE id = ?",
                java.sql.Timestamp.from(when), cardId);
    }
}
