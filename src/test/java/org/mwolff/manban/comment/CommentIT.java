...
class CommentIT extends AbstractIntegrationTest {
  ...
  void authorCanCrudOwnComment() throws Exception {
    ...
    mvc.perform(
            patch("/api/comments/" + commentId)
                .cookie(alice)
                .contentType("application/json")
                .content("{\"body\":\"Bearbeitet\"}"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.body").value("Bearbeitet"));

    mvc.perform(delete("/api/comments/" + commentId).cookie(alice))
  ...
  void foreignCommentCannotBeEditedByOtherMember() throws Exception {
    ...
    memberships.save(
        new ProjectMembership(
            null, projectId, userId("cm-bob@example.com"), ProjectRole.MEMBER, Instant.now()));
    memberships.save(
        new ProjectMembership(
            null, projectId, userId("cm-carol@example.com"), ProjectRole.MEMBER, Instant.now()));

    long bobComment = createComment(bob, cardId, "Bobs Kommentar");

    mvc.perform(
            patch("/api/comments/" + bobComment)
                .cookie(carol)
                .contentType("application/json")
                .content("{\"body\":\"gekapert\"}"))
  ...
  void ownerDeletesForeignCommentButCannotEditIt() throws Exception {
    ...
    memberships.save(
        new ProjectMembership(
            null, projectId, userId("cm-mod-bob@example.com"), ProjectRole.MEMBER, Instant.now()));
  ...
  void memberAuthorEditsOwnButCannotDeleteOnlyAdmin() throws Exception {
    ...
    memberships.save(
        new ProjectMembership(
            null, projectId, userId("cm-md-bob@example.com"), ProjectRole.MEMBER, Instant.now()));
  ...
  void viewerCannotComment() throws Exception {
    ...
    memberships.save(
        new ProjectMembership(
            null, projectId, userId("cm-viewer@example.com"), ProjectRole.VIEWER, Instant.now()));
