 ...
 *
 * <p>Die Regeln bilden die real vorhandene, saubere Architektur ab: Basis-Package
 * {@code org.mwolff.manban}, Fachmodule mit den Sub-Packages {@code domain}, {@code application},
 * {@code web}, {@code infrastructure}. Das Domänenmodell ist framework-frei; JPA-Entities liegen in
 * {@code infrastructure}, Controller in {@code web}.
 *
 * <p>Bewusst nicht über die {@code archunit-junit5}-Engine ({@code @AnalyzeClasses}/{@code @ArchTest})
 * ausgeführt: Maven Surefire 3.5.3 registriert die ArchUnit-TestEngine in diesem Projekt nicht (die
 * Regeln liefen dann als „0 Tests" durch, ein bewusst eingebauter Verstoß blieb unentdeckt). Stattdessen
 * werden die Klassen einmalig via {@link ClassFileImporter} (ohne Testklassen, entspricht
 * {@code DoNotIncludeTests}) importiert und die Regeln über reguläre JUnit-Jupiter-Tests geprüft — so
