...
class KanbanCompatIT {
    ...
    private long firstColumnId(Cookie session, long boardId) throws Exception {
        String body = mvc.perform(get("/api/boards/" + boardId).cookie(session))
                .andExpect(status().isOk())
                .andReturn().getResponse().getContentAsString();
    ...
    private String boundToken(Cookie session, long projectId, long boardId) throws Exception {
        String body = mvc.perform(post("/api/access-tokens").cookie(session)
                        .contentType("application/json")
                        .content("{\"name\":\"board-token\",\"projectId\":%d,\"boardId\":%d}"
    ...
    private JsonNode kanbanItems(String token) throws Exception {
        String body = mvc.perform(get("/api/kanban/items").header("X-Kanban-Token", token))
                .andExpect(status().isOk())
                .andReturn().getResponse().getContentAsString();
    ...
    void fullClientFlowCreateListMoveComment() throws Exception {
        ...
        // create -> {number}
        String created = mvc.perform(post("/api/kanban/items").header("X-Kanban-Token", token)
                        .contentType("application/json")
                        .content("{\"title\":\"Erste Aufgabe\",\"body\":\"Beschreibung\",\"column\":\"BACKLOG\"}"))
                .andExpect(status().isCreated())
        ...
        // move -> IN_PROGRESS
        mvc.perform(put("/api/kanban/items/" + id + "/move").header("X-Kanban-Token", token)
                        .contentType("application/json")
        ...
        // comment
        mvc.perform(post("/api/kanban/items/" + id + "/comments").header("X-Kanban-Token", token)
                        .contentType("application/json").content("{\"body\":\"Ein Kommentar\"}"))
                .andExpect(status().isCreated());
    }
    ...
    void epicsEndpointReportsProgress() throws Exception {
        ...
        mvc.perform(post("/api/boards/" + boardId + "/cards").cookie(session)
                        .contentType("application/json")
    ...
    void unboundTokenIsConflict() throws Exception {
        ...
        String body = mvc.perform(post("/api/access-tokens").cookie(session)
                        .contentType("application/json").content("{\"name\":\"unbound\"}"))
                .andExpect(status().isCreated())
                .andReturn().getResponse().getContentAsString();
    ...
    void tokenIsScopedToItsBoard() throws Exception {
        ...
        String createdOnBoard2 = mvc.perform(post("/api/boards/" + board2 + "/cards").cookie(session)
                        .contentType("application/json")
        ...
        mvc.perform(put("/api/kanban/items/" + foreignCardId + "/move").header("X-Kanban-Token", token1)
                        .contentType("application/json").content("{\"column\":\"DONE\",\"position\":0}"))
        ...
        for (String col : new String[] {"BACKLOG", "READY", "IN_PROGRESS", "IN_REVIEW", "DONE"}) {
            assertThat(items.get(col)).allSatisfy(n -> assertThat(n.get("title").asText()).isNotEqualTo("Fremd"));
        }
    }
}
